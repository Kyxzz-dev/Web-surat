    ...
    <script>
        $(document).on('click', '.btn-edit', function () {
            ...
            if ($(this).data('active') == 1) {
                $('#editModal input#xis_active').attr('checked', 1)
            } else {
                $('#editModal input#is_active').removeAttribute('checked');
            }
              const role = $(this).closest('tr').find('.badge').text().trim().toLowerCase();
    $('#editModal select#role').val(role);
        });
    </script>
@endpush
...
<div class="mt-3 px-4 pb-3">
    <div class="mb-0" role="alert" style="background-color: #fff4e5; border-left: 4px solid #f58b00; padding: 10px 15px; border-radius: 4px; font-size: 0.9rem; color: #663c00;">
        ...
        <ul class="mb-0 mt-2" style="padding-left: 20px;">
            <li>
                Untuk menonaktifkan akun, klik tombol <strong>Edit</strong> pada akun yang dimaksud, 
                lalu hilangkan centang pada opsi <strong>"Masih aktif?"</strong>.
            </li>
            <li class="mt-2">
                Untuk menjadikan akun sebagai "<strong>Admin</strong>", klik tombol <strong>Edit</strong> pada akun yang dimaksud, 
                lalu centang opsi <strong>"Masih aktif?"</strong> dan pilih Peran "<strong>ADMIN</strong>".
            </li>
        </ul>
    </div>
</div>
        ...
        <div class="modal-dialog">
            <form class="modal-content" method="post" action="">
                ...
                <div class="modal-body">
                    ...
                    <label>{{ __('model.user.role') }}</label>
